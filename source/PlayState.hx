...
class PlayState extends MusicBeatState
{
	...
	function getActorByName(id:String):Dynamic
	{
		...
		if (luaSprites.get(id) == null)
			return enemyStrums.members[Std.parseInt(id)];
	...
	{
		#if sys
		var data:BitmapData = BitmapData.fromFile(Sys.getCwd() + "scripts/modcharts" + PlayState.SONG.song.toLowerCase() + '/' + spritePath + ".png");
	...
	override public function create(){
		...
		#if sys
		executeModchart = FileSystem.exists(Paths.lua(PlayState.SONG.song.toLowerCase()));
		#end
		#if !cpp
		executeModchart = false; // FORCE disable for non cpp targets
		#end

		trace('Mod chart: ' + executeModchart + " - " + Paths.lua(PlayState.SONG.song.toLowerCase()));
		...
		*/
		...
		var luaFile:String = 'scripts/songs/' + SONG.song + '.lua';
		{
			if(FileSystem.exists(luaFile)) {
				doPush = true;
			}
		}
		var luaFile2:String = 'scripts/stages/' + curStage + '.lua';
		{
			if(FileSystem.exists(luaFile2)) {
				doPush = true;
			}
		}
		var luaFile3:String = 'scripts/characters/' + SONG.player2 + '.lua';
		{
			if(FileSystem.exists(luaFile3)) {
				doPush = true;
			}
		}
		var luaFile4:String = 'scripts/characters/' + SONG.player1 + '.lua';
		{
			if(FileSystem.exists(luaFile4)) {
				doPush = true;
			}
		}
		if(doPush) {
			...
			luaArray.push(new FunkinLua(luaFile3));
			luaArray.push(new FunkinLua(luaFile4));
		}

		var filesPushed:Array<String> = [];
		var foldersToCheck:Array<String> = ['scripts/global/'];
		...
		healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this, 'healthLerp', 0, 100);
		...
		if(Utils.exists("scripts/songs/" + SONG.song.toLowerCase() + ".chainsaw")
		|| Utils.exists("scripts/songs/" + SONG.song.toLowerCase() + ".hscript")
		|| Utils.exists("scripts/songs/" + SONG.song.toLowerCase() + ".hx")
		|| Utils.exists("scripts/songs/" + SONG.song.toLowerCase() + ".hxc")
		|| Utils.exists("scripts/songs/" + SONG.song.toLowerCase() + ".hxs"))
		{

		}

		if(Utils.exists("scripts/stages/" + curStage.toLowerCase() + ".chainsaw")
		|| Utils.exists("scripts/stages/" + curStage.toLowerCase() + ".hscript")
		|| Utils.exists("scripts/stages/" + curStage.toLowerCase() + ".hx")
		|| Utils.exists("scripts/stages/" + curStage.toLowerCase() + ".hxc")
		|| Utils.exists("scripts/stages/" + curStage.toLowerCase() + ".hxs"))
		{
	
		}

		if(Utils.exists("scripts/characters/" + SONG.player2.toLowerCase() + ".chainsaw")
		|| Utils.exists("scripts/characters/" + SONG.player2.toLowerCase() + ".hscript")
		|| Utils.exists("scripts/characters/" + SONG.player2.toLowerCase() + ".hx")
		|| Utils.exists("scripts/characters/" + SONG.player2.toLowerCase() + ".hxc")
		|| Utils.exists("scripts/characters/" + SONG.player2.toLowerCase() + ".hxs"))
		{
		
		}

		if(Utils.exists("scripts/characters/" + SONG.player1.toLowerCase() + ".chainsaw")
		|| Utils.exists("scripts/characters/" + SONG.player1.toLowerCase() + ".hscript")
		|| Utils.exists("scripts/characters/" + SONG.player1.toLowerCase() + ".hx")
		|| Utils.exists("scripts/characters/" + SONG.player1.toLowerCase() + ".hxc")
		|| Utils.exists("scripts/characters/" + SONG.player1.toLowerCase() + ".hxs"))
		{
	...
	public function startCountdown():Void {
		...
		for (i in 0...enemyStrums.length) {
			setOnLuas('defaultOpponentStrumX' + i, enemyStrums.members[i].x);
			setOnLuas('defaultOpponentStrumY' + i, enemyStrums.members[i].y);
		}
		...
		if (executeModchart) // dude I hate lua (jkjkjkjk)
			{
				...
				var result = LuaL.dofile(lua, Paths.lua(PlayState.SONG.song.toLowerCase())); // execute le file
				...
				for (i in 0...enemyStrums.length) {
					var member = enemyStrums.members[i];
					trace(enemyStrums.members[i].x + " " + enemyStrums.members[i].y + " " + enemyStrums.members[i].angle + " | strum" + i);
	...
	public function generateStaticArrows(player:Int, ?instant:Bool = false, ?skin:String):Void{
		...
		for (i in 0...4){
			...
			if(Config.noteColor == 1){
				babyArrow.color = FlxColor.RED;
			}else if(Config.noteColor == 2){
				babyArrow.color = FlxColor.GREEN;
			}else if(Config.noteColor == 0){
				babyArrow.color = FlxColor.WHITE;
			}else if(Config.noteColor == 3){
				babyArrow.color = FlxColor.YELLOW;
			}else if(Config.noteColor == 4){
				babyArrow.color = FlxColor.BLACK;
			}else if(Config.noteColor == 5){
				babyArrow.color = FlxColor.BLUE;
			}else if(Config.noteColor == 6){
				babyArrow.color = FlxColor.PURPLE;
			}else if(Config.noteColor == 7){
				babyArrow.color = FlxColor.BROWN;
			}else if(Config.noteColor == 8){
				babyArrow.color = FlxColor.CYAN;
			}else if(Config.noteColor == 9){
				babyArrow.color = FlxColor.MAGENTA;
			}else if(Config.noteColor == 10){
				babyArrow.color = FlxColor.LIME;
			}else if(Config.noteColor == 11){
				babyArrow.color = FlxColor.ORANGE;
			}
	...
	override public function update(elapsed:Float) {
		...
		if (FlxG.keys.justPressed.SIX){
			...
			PlayState.botplayTxt.visible = !PlayState.botplayTxt.visible;
	...
	public function endSong():Void{
		...
		if (isStoryMode){
			...
			if (storyPlaylist.length <= 0)
			{
				...
				if (SONG.player1 == 'PicoAll'){
					switchState(new ResultsState(weekStats, StoryMenuState.weekNamesShort[storyWeek], "Pico", songSaveStuff));
				}else{
					switchState(new ResultsState(weekStats, StoryMenuState.weekNamesShort[storyWeek], "Boyfriend", songSaveStuff));
				}
		...
		//CODE FOR ENDING A FREEPLAY SONG
			...
			if (SONG.player1 == 'PicoAll'){
				switchState(new ResultsState(songStats, songName, "Pico", songSaveStuff));
			}else{
				switchState(new ResultsState(songStats, songName, "Boyfriend", songSaveStuff));
			}
		}
	}
	...
	function updateScoreText(){
		...
		var scoreComma:String = FlxStringUtil.formatMoney(songStats.score, false, true);
		var comboComma:String = FlxStringUtil.formatMoney(combo, false, true);
		var missComma:String = FlxStringUtil.formatMoney(songStats.missCount, false, true);
		var comboBreakComma:String = FlxStringUtil.formatMoney(songStats.comboBreakCount, false, true);
		...
		scoreTxt.text = "Score: " + scoreComma;
		{
			if(Config.showMisses == 1){
				...
				scoreTxt.text += " • Combo Breaks: " + comboBreakComma;
			}
	
			scoreTxt.text += " • Combo: " + comboComma;

			scoreTxt.text += " • Health: " + Math.round(health) + "%";
	
			if(Config.showAccuracy){
				scoreTxt.text += " • Rating: " + Math.round(truncateFloat(songStats.accuracy, 2)) + "%";
			}
		}
