...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	override function create() {
		...
		artwork = new FlxSprite().loadGraphic(Paths.image('FREEPLAY/ARTWORK/BF'));
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			weekFile.songs[curSelected][3] = artworkInputText.text;
			changeArtwork();
	...
	var artworkInputText:FlxUIInputText;
	function addFreeplayUI() {
		...
		artworkInputText = new FlxUIInputText(140, bgColorStepperR.y + 70, 100, '', 8);

		blockPressWhileTypingOn.push(iconInputText);
		blockPressWhileTypingOn.push(artworkInputText);
		...
		tab_group.add(new FlxText(140, artworkInputText.y - 18, 0, 'Selected artwork:'));
		...
		tab_group.add(artworkInputText);
	...
	function changeSelection(change:Int = 0) {
		...
		artworkInputText.text = weekFile.songs[curSelected][3];
	...
	function changeArtwork(){
		remove(artwork);
		artwork = new FlxSprite().loadGraphic(Paths.image('FREEPLAY/ARTWORK/' + weekFile.songs[curSelected][3]));
		artwork.antialiasing = ClientPrefs.data.antialiasing;
		artwork.x = 7000;
		artwork.alpha = 0;
		add(artwork);
		artwork.screenCenter();
		FlxTween.tween(artwork, {alpha: 1}, 0.4, {ease: FlxEase.quartInOut});
		FlxTween.tween(artwork, {x: 650}, 0.3, {ease: FlxEase.expoOut});
	}
