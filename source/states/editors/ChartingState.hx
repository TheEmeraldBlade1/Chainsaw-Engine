...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.justPressed || FlxG.mouse.justPressedRight || FlxG.mouse.justPressedMiddle || FlxG.mouse.pressed || FlxG.mouse.pressedRight || FlxG.mouse.pressedMiddle)
		{
			if (FlxG.mouse.overlaps(curRenderedNotes))
			{
				curRenderedNotes.forEachAlive(function(note:Note)
				{
					...
					{
						if (FlxG.mouse.justPressedRight || FlxG.mouse.pressedRight)
						{
							deleteNote(note);
						}
						else if (FlxG.mouse.justPressedMiddle || FlxG.mouse.pressedMiddle)
						{
							selectNote(note);
							curSelectedNote[3] = curNoteTypes[currentType];
							updateGrid();
