...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'healthBar', function() return health, 0, 100);
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		if (ClientPrefs.data.uiType == 'Micd Up'){
			...
			if (ratingName != '?'){
				accuracyTxt.text = 'Accuracy: ' + CoolUtil.floorDecimal(ratingPercent * 100, 2) + '%';
			}else{
	...
	override public function update(elapsed:Float)
	{
		...
		if(!endingSong && !inCutscene && allowDebugKeys)
		{
			if (FlxG.keys.justPressed.SEVEN)
				...
				openCharacterEditor(SONG.player2);
			else if (FlxG.keys.justPressed.NINE)
				openCharacterEditor(SONG.player1);
			else if (FlxG.keys.justPressed.FIVE)
				openCharacterEditor(SONG.gfVersion);
		}
	...
	function openCharacterEditor(variableCharacterString:String)
	{
		FlxG.camera.followLerp = 0;
		persistentUpdate = false;
		paused = true;
		...
		MusicBeatState.switchState(new CharacterEditorState(variableCharacterString));
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		strumPlayAnim(true, Std.int(Math.abs(note.noteData)), 0.05);
